...
const CreateNewsScreen = () => {
  ...
  const fetchUser = async () => {
    try {
      const response = await fetch("http://172.20.10.2:8080/user/auth/user", { method: "GET" });
  ...
  const fetchCategories = async () => {
    try {
      const response = await fetch('http://172.20.10.2:8080/category/get', { method: 'GET' });
      const data = await response.json();
  ...
  const handleNewsSubmit = async () => {
    try {
      const response = await fetch('http://172.20.10.2:8080/news/add', {
        method: "POST",
        headers: {
